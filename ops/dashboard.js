...
/* Разметка отчёта приходит из Markdown. Полноценный разборщик здесь
   не нужен и вреден: отчёт пишет свой же код, набор конструкций в нём
   известен и узок. Экранирование при этом обязательно и идёт ПЕРВЫМ —
   в отчёт попадают заголовки новостей и сообщения об ошибках, то есть
   текст, который пишем не мы. Экранирует общий `esc` из начала файла. */
function mdLite(src) {
  ...
  for (let raw of lines) {
    ...
    if (t === "---" || t === "") { if (list) { out.push("</ul>"); list = false; } continue; }
    const li = t.match(/^[-*]\s+(.*)$/);
    if (li) {
      if (!list) { out.push('<ul class="an-list">'); list = true; }
      out.push("<li>" + inline(li[1]) + "</li>");
      continue;
    }
    if (list) { out.push("</ul>"); list = false; }
    const h = t.match(/^(#{1,4})\s+(.*)$/);
    if (h) { out.push(`<h${h[1].length + 2} class="an-h">${inline(h[2])}</h${h[1].length + 2}>`); continue; }
    if (t.startsWith("&gt;")) { out.push(`<blockquote>${inline(t.slice(4).trim())}</blockquote>`); continue; }
...
let anBusy = false;
let anOpen = false;
let anSig = "";   /* слепок состояния: карточка перерисовывается только
                     когда есть что перерисовывать. Панель опрашивается
                     раз в секунду, и безусловная перерисовка стирала бы
                     сообщение о запуске и сбрасывала прокрутку отчёта
                     под руками у читающего. */

async function anPost(action) {
  if (anBusy) return;
  anBusy = true;
  const msg = $("an-msg");
  if (msg) msg.textContent = "Запускаю…";
  try {
    const r = await fetch("/api/analyst/" + action, {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: "{}"
    });
    const d = await r.json();
    if (msg) {
      /* О времени честно. Первый разбор тянет за собой проверку
         качества — полтора десятка вопросов к модели, — и это
         десятки минут, а не «несколько». Человек, которому обещали
         минуту, через пять решит, что всё зависло, и убьёт процесс. */
      const how = action === "train" ? "Пара секунд."
        : action === "check" ? "Проверка качества — это 15 вопросов к модели, "
          + "обычно 10–25 минут."
        : "Если качество ещё не подтверждено, сначала пройдёт проверка "
          + "(10–25 минут), потом сам разбор.";
      msg.textContent = r.ok
        ? "Запущено. " + how + " Карточка обновится сама."
        : "Не вышло: " + (d.error || r.status);
    }
  } catch (e) {
    if (msg) msg.textContent = "Панель не достучалась: " + e.message;
  } finally {
    anBusy = false;
  }
}

function renderAnalyst(s, force) {
  const a = s.analyst || {};
  const node = $("analyst");
  if (!node) return;

  const sig = [a.available, a.busy, a.state, a.ready, a.eval_score,
               a.grounding, a.last_run_utc, a.ts_utc, anOpen,
               a.report && a.report.age_hours].join("|");
  if (!force && sig === anSig) return;
  anSig = sig;

  if (!a.available && a.ready == null) {
    node.innerHTML = `<div class="empty">Ни разу не запускался. Запустить:
      <code>.venv\\Scripts\\python.exe ops\\analyst.py run</code></div>`;
    return;
  }
  ...
  const verdict = a.busy
    ? [`работает: ${esc(a.state)}`, "var(--accent)"]
    : ready
    ? ["готов к разбору", "var(--good)"]
    : ["не готов", "var(--critical)"];

  const stat = `
    <div class="pstat">
      <div><b style="color:${verdict[1]};font-size:15px;font-weight:500">${verdict[0]}</b>
           <span>состояние</span></div>
      <div><b>${a.eval_score == null ? "—" : fmt(a.eval_score, 2)}</b>
           <span>оценка качества</span></div>
      <div><b>${a.grounding == null ? "—" : fmt(a.grounding * 100, 0) + "%"}</b>
           <span>обоснованность</span></div>
      <div><b style="font-size:13px;font-weight:500">${esc(a.model || "—")}</b>
           <span>модель${a.backend ? " · " + esc(a.backend) : ""}</span></div>
      <div><b style="font-size:13px;font-weight:500">${a.report
             ? (a.report.age_hours < 1 ? "только что"
                : fmt(a.report.age_hours, 0) + " ч назад") : "нет"}</b>
           <span>последний отчёт</span></div>
    </div>`;

  const rows = checks.map(c => `
    <div class="check">
      <div><div class="lab">${esc(c.label)}</div></div>
      <div class="v"><span class="cap">итог</span>
        <span style="color:${c.ok ? "var(--good)"
          : c.blocking ? "var(--critical)" : "var(--ink-2)"}">
          ${c.ok ? "да" : c.blocking ? "НЕТ" : "нет, но не мешает"}</span></div>
      <div class="v real"><span class="cap">подробности</span>${esc(c.detail)}</div>
      <div class="note">${c.ok ? "" : esc(c.fix || "")}</div>
    </div>`).join("");

  const problems = blockers.length
    ? `<p class="hint" style="color:var(--critical);margin:12px 0 0">
         Разбор не запустится, пока не исправлено:
         ${blockers.map(b => esc(b.label)).join("; ")}.</p>`
    : warns.length
    ? `<p class="hint" style="margin:12px 0 0">Оговорки к выводам:
         ${warns.map(w => esc(w.label) + " — " + esc(w.detail)).join("; ")}.</p>`
    : "";

  const buttons = `
    <div class="chips" style="margin-top:14px;gap:8px">
      <button class="act" id="an-run" ${a.busy ? "disabled" : ""}>
        ${a.busy ? "Идёт " + esc(a.state) + "…" : "Запустить разбор"}</button>
      <button class="ghost" id="an-train" ${a.busy ? "disabled" : ""}>Обучить</button>
      <button class="ghost" id="an-check" ${a.busy ? "disabled" : ""}>Проверить качество</button>
      ${a.report ? `<button class="ghost" id="an-toggle">
        ${anOpen ? "Свернуть отчёт" : "Показать отчёт"}</button>` : ""}
    </div>
    <div id="an-msg" class="hint" style="margin-top:8px"></div>`;

  const body = (anOpen && a.report)
    ? `<div class="an-report">${mdLite(a.report.text)}</div>` : "";

  node.innerHTML = stat + rows + problems + buttons + body;

  const bind = (id, fn) => { const n = $(id); if (n) n.onclick = fn; };
  bind("an-run", () => anPost("run"));
  bind("an-train", () => anPost("train"));
  bind("an-check", () => anPost("check"));
  bind("an-toggle", () => { anOpen = !anOpen; renderAnalyst(s, true); });
}
